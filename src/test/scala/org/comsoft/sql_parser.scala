...
object sql_parser extends App {
  ...
  def traverse(list: List[String], l1: List[String], l2: List[String], l3: List[String]): (List[String], List[String], List[String]) = {
    list match {
      ...
      case y :: ys =>
        y match {
          case x if x.matches("(?s).*CREATE *GENERATOR.*") => traverse(ys, x :: l1, l2, l3)
          case x if x.matches("(?s).*CREATE *TABLE.*") => traverse(ys, l1, x :: l2, l3)
          case x if x.matches("(?s).*CREATE *INDEX.*") => traverse(ys, l1, l2, x :: l3)
          case x if x.matches("(?s).*ALTER *TABLE.*") => traverse(ys, l1, l2, x :: l3)
